...
import { X, Plus, Shuffle, Trash2, ChevronDown, ChevronRight, Pencil, Check } from 'lucide-react';
...
export function CharacterPanel() {
  ...
  const { characters, addCharacter, removeCharacter, updateCharacter } = useEditorStore();
  ...
  const handleAdd = () => {
    if (!name.trim()) return;
    addCharacter({ name: name.trim(), role: role.trim() || t.characters.defaultRole, color });
    setName('');
    setRole('');
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-4 pt-5 pb-3 border-b border-zinc-100 dark:border-zinc-800">
        <h2 className="font-semibold text-sm text-zinc-800 dark:text-zinc-200 mb-3">{t.characters.title}</h2>
        <input
          value={search}
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-1.5">
        {filtered.length === 0 && <p className="text-xs text-zinc-400 text-center mt-10">{t.characters.noCharacters}</p>}
        {filtered.map(char => (
          <CharacterCard 
            key={char.id} 
            char={char} 
      ...
      <div className="px-4 pb-5 pt-3 border-t border-zinc-100 dark:border-zinc-800 space-y-2.5">
        ...
        <div className="flex gap-2">
          <input value={name} onChange={e => setName(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAdd()}
            placeholder={t.characters.addName}
        ...
        <input value={role} onChange={e => setRole(e.target.value)}
          placeholder={t.characters.addRole}
          className="w-full text-sm bg-zinc-50 dark:bg-zinc-800/60 border border-zinc-200 dark:border-zinc-700 rounded-lg px-3 py-1.5 outline-none focus:border-zinc-400 transition-colors text-zinc-700 dark:text-zinc-300" />
...
function CharacterCard({ char, onRemove, onUpdate }: { char: Character; onRemove: () => void; onUpdate: (u: Partial<Character>) => void }) {
  const { t } = useTranslation();
  ...
  const handleSaveEdit = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (editName.trim()) {
      onUpdate({ name: editName.trim(), role: editRole.trim() });
    }
    setIsEditing(false);
  };
        ...
        <div className="flex items-center gap-1">
          {isEditing ? (
            <button onClick={handleSaveEdit}
              className="p-1 rounded-md bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 hover:opacity-80 transition-all">
        ...
        <div className="border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/40 px-3 py-2.5 space-y-2">
          ...
          <div className="flex gap-1.5">
            <input
              ...
              placeholder={t.characters.detailKey}
            ...
            <AutoTextarea
              ...
              placeholder={t.characters.detailValue}
