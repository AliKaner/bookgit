...
// Categories are now managed via t.world.categories
...
export function WorldPanel() {
  ...
  const categories = Object.values(t.world.categories);
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="px-4 pt-5 pb-3 border-b border-zinc-100 dark:border-zinc-800">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h2 className="font-semibold text-sm text-zinc-800 dark:text-zinc-200">{t.world.title}</h2>
        </div>
        <p className="text-xs text-zinc-400 mb-3">{t.world.subtitle}</p>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
        {world.length === 0 && (
          <p className="text-xs text-zinc-400 text-center mt-8">{t.world.noEntries}</p>
        )}
        {world.map(entry => (
          <WorldCard
            key={entry.id}
