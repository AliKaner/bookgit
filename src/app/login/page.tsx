...
export default function LoginPage() {
  ...
  // Handle ?error= from /auth/callback failures
  useEffect(() => {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      if (tab === "signin") {
        ...
        if (!result) return; // redirected (email confirmation off)
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-950 px-4">
        ...
        <div className="relative w-full max-w-sm text-center">
          ...
          <p className="text-sm font-semibold text-violet-300 mb-6 bg-violet-900/20 border border-violet-800/40 rounded-lg px-3 py-2 inline-block">
            {sentEmail}
          </p>

          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 text-left space-y-3 mb-6">
            {[
              "Open the email from BookGit",
          ...
          <div className="flex items-center justify-center gap-3">
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-lg border border-zinc-700 text-zinc-400 text-xs hover:bg-zinc-800 hover:text-white transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {resendPending
            ...
            <button
              onClick={() => { setStage("form"); setTab("signin"); setError(null); }}
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8 shadow-2xl shadow-black/50">
            ...
            {(["signin", "signup"] as Tab[]).map((tabKey) => (
              <button key={tabKey} onClick={() => { setTab(tabKey); setError(null); }}
                className={`flex-1 py-1.5 rounded-md text-sm font-medium transition-all ${
                  tab === tabKey ? "bg-white text-zinc-900 shadow-sm" : "text-zinc-400 hover:text-zinc-200"
                }`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              {tab === "signup" && (
                <p className="text-[10px] text-zinc-600 mt-1">{t.auth.minPassword}</p>
              )}
            </div>
